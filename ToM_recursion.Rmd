...
prepare <- function(strategy_string, RB_prop = 0.5){ 
  ...
  if (strategy_string == "RB") {
    ...
    stop("k-tom does not work yet... dumbass..  sorry, but come on!")
...
prepare_df <- function(agent_df_unprep, args = NULL){ 
  ...
  tmp1 <- sapply(agent_df_unprep$player1, prepare)
  tmp2 <- sapply(agent_df_unprep$player2, prepare)
    #save params
  agent_df_unprep$params_p1 <- t(tmp1)[,1]
  agent_df_unprep$params_p2 <- t(tmp2)[,1]
    #Save hidden states
  agent_df_unprep$hidden_states_p1 <- t(tmp1)[,2]
  agent_df_unprep$hidden_states_p2 <- t(tmp2)[,2]
...
compete <- function(player1, player2, p1_params, p2_params, p1_hidden_states = NA, p2_hidden_states = NA, p_matrix, n_rounds = 10){ 
  ...
  for (round in 1:n_rounds){
    ...
    p2 <- do.call(strat_p2, 
      ...
      #generate result
    result <- p_matrix[p_matrix$a1_choice == p1$choice & p_matrix$a2_choice == p2$choice,]

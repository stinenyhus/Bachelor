...
#Expected payoff
payoff_difference <- function(p_op_1){
  p_op_1*(U(1,1)-U(0,1))+(1-p_op_1)*(U(1,0)-U(0,0))
}
...
p_op_1 <- function(mu, variance, sigma){
...
p_op_1_k <- function(mu, variance){
  #INPUT
    #takes a mu and a variance (of opponent)
  #OUTPUT
    #p_op_1_k probability of opponent choosing 1 given k, e.g. P(c_op = 1|k')
...
ID = 1
last_round <- NA
params = agents_df$params[agents_df$ID == ID] #!# temp for testing
params[[1]]
...
k_ToM <- function(ID, last_round, params = agents_df$params[agents_df$ID == ID]){
  ...
  ###_____ FIRST ROUND _____###
  # if (class(last_round)[1] != "tbl_df"){ #first round
  #   if (k != 0){
  #     stop("this is not made yet")
  #   }
  # }
  ...
  if (class(last_round)[1] == "tbl_df"){ #not first roundfirst round
    if (k == 0){ #0-ToM
      ...
      mu <- mu + p_k * variance * (ot - p_op_1_k) #posterior mean (mu)
      ...
      p_op_1_k <- p_op_1_k(mu, variance) #P(c_op = 1 | k'), prob. of op. choosing 1 given sophistication level, k'
    }
  }
  ...
  if (k == 0){ #0-ToM
    p_op_1 <- p_op_1(mu, variance, sigma) #estimated probability of opponent chosing 1 in %
  } else { #k > 0
...
#testing code
```{r} 
agents_df <- head(agents_df, 2)
agents_df$strategy = c("0-ToM", "1-ToM")
agents_df$strategy_function = rep("k-ToM", 2)
agents_df$params <- NA
agents_df

agents_df$params[1] <- prepare_kToM(0)
agents_df$params[2] <- prepare_kToM(1)

prepare_kToM(2)


temp <- compete(c(1,3), n_rounds = 30)

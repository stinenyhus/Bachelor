...
Fix the transition from param_mean to params_opponent
Summing variance - vector/matrix question
  ...
  if (level == 0) { #If the (simulated) agent is a 0-ToM
    ...
    for (level_index in 1:level) {
      ...
      gradient[level_index] = gradient_update(opponent_prev_hidden_states = opponent_hidden_states,
                                 ...
                                 param_mean = param_mean[level_index],
...
p_op_1_k_fun <- function(hidden_states){
  ...
  #Prepare variance
  variance = sum(exp(variance) * gradient^2) #!# how does sum work on a matrix
...
#Testing Space
```{r}
test = rec_prepare_k_ToM(level = 3)

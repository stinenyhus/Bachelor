...
WSLS <- function(ID, player, p_matrix, last_round, 
  ...
  temperature_win <- params$WSLS$temperature_win
  temperature_loose <- params$WSLS$temperature_loose
  ...
  if (class(last_round)[1] != "tbl_df") { #initial round or missed trial
    ...
    if (gain - mean_gain > 0) { #if the player won last round
      ...
      # Input gain relative to maximum gain in a logistic function. Lower noise param randomizes. 
      probability_stay = 1 / (1 + exp(-(gain/max_gain - mean_gain/max_gain)/temperature_win))  #When temperature->0, the agent always stays if above mean score.
      ...
      probability_switch = 1 / (1 + exp(-(min_gain/gain  - min_gain/mean_gain)/temperature_loose)) #When temperature->0, the agent always switches if below mean score.

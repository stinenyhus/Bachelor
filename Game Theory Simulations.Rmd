...
agent_df <- create_agents(c("0-ToM", "1-ToM"), match_up = "RR", prepare_with_defaults = T, progr)
tmp_df <- compete_all(agent_df, n_rounds = 300, 
...
strategies_unprep <- c("RB", "WSLS", "TFT", "0-ToM", "1-ToM", "2-ToM", "3-ToM", "4-ToM", "5-ToM", "6-ToM", "7-ToM")
...
result_df <- compete_all(agent_df, n_rounds = 200, 
        p_matrix = penny_competitive_matrix)

#save
#save(result_df, file = "result_df_full.rda")
...
k_ToM_params_distribution =
  c(volatility_mean = -2,
    volatility_sd = 1,
    temperature_mean = -1,
    temperature_sd = 1)

#Make the agents with the distributed parameter values
agent_df_unprep_distribution <- create_agents(strategies_unprep, match_up = "RR", prepare_with_defaults = F)

#Give agents their parameters and priors. Use the self-similar priors for k-ToM.
agent_df_distribution <- prepare_df(agent_df_unprep_distribution, args = rep("k_ToM = k_ToM_params_distribution", nrow(agent_df_unprep_distribution)*2))

#Make all agents in the dataframe compete
result_df_distribution <- compete_all(agent_df_distribution, n_rounds = 200, 
        p_matrix = penny_competitive_matrix)

#Save the results
save(result_df_distribution, file = "result_df_distribution.rda")
rm(result_df_distribution)
...
#make list of sim numbers to add to finished df's
sim_nr <- sort(rep(seq(1,nsims),2*nrounds))

### RB v 0ToM

#set strategies
strategies_unprep <- c("RB", "0-ToM")

result_df_multiple <- NULL

#Run the competition as nsim times. Save it in one dataset
for (sim in 1:nsims) {
  agent_df <- create_agents(strategies_unprep, match_up = "RR", prepare_with_defaults = T)
  result_df_single <- compete_all(agent_df, n_rounds = nrounds, 
        p_matrix = penny_competitive_matrix)
  result_df_multiple <- rbind(result_df_multiple, result_df_single)
}

#Proper variable name
result_df_RB_0ToM <- result_df_multiple
#Add simulation number
result_df_RB_0ToM$sim <- sim_nr
#Save
save(result_df_RB_0ToM, file = "result_df_RB_0ToM.rda")
rm(result_df_RB_0ToM)
...
#save the results
result_df_0ToM_1ToM <- result_df_multiple
result_df_0ToM_1ToM$sim <- sim_nr
save(result_df_0ToM_1ToM, file = "result_df_0ToM_1ToM.rda")
rm(result_df_0ToM_1ToM)


### --- ###
### 1ToM v 2ToM
#set strategies
strategies_unprep <- c("1-ToM", "2-ToM")
...
#save the results
result_df_1ToM_2ToM <- result_df_multiple
result_df_1ToM_2ToM$sim <- sim_nr
save(result_df_1ToM_2ToM, file = "result_df_1ToM_2ToM.rda")
rm(result_df_1ToM_2ToM)


### --- ###
### 0ToM v 2ToM
#set strategies
strategies_unprep <- c("0-ToM", "2-ToM")
...
#save the results
result_df_0ToM_2ToM <- result_df_multiple
result_df_0ToM_2ToM$sim <- sim_nr
save(result_df_0ToM_2ToM, file = "result_df_0ToM_2ToM.rda")
rm(result_df_0ToM_2ToM)

### --- ###
### 2ToM v 5ToM
#set strategies
strategies_unprep <- c("2-ToM", "5-ToM")
...
#save the results
result_df_2ToM_5ToM <- result_df_multiple
result_df_2ToM_5ToM$sim <- sim_nr
save(result_df_2ToM_5ToM, file = "result_df_2ToM_5ToM.rda")
rm(result_df_2ToM_5ToM)
...
### 4ToM v 5ToM
#set strategies
strategies_unprep <- c("4-ToM", "5-ToM")
...
#save the results
result_df_4ToM_5ToM <- result_df_multiple
result_df_4ToM_5ToM$sim <- sim_nr
save(result_df_4ToM_5ToM, file = "result_df_4ToM_5ToM.rda")
rm(result_df_4ToM_5ToM)

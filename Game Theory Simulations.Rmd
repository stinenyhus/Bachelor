...
#Nice to do:
  ...
  Make sure the TFT and the WSLS isn't doing the same thing 
...
###ALTERNATIVE VERSION #!# We should test this out at some point ;) 
...
Prepare k-ToM
```{r}
prepare_k_ToM <- function(k, priors = "default", params = "default"){ 
  #This function calls the recursive preparation function for k-ToM to prepare all possible hidden states and then 
  ...
  hidden_states <- rec_prepare_k_ToM(level = k, priors = "default")
  
  return(list(params,hidden_states))
}
```
...
prepare <- function(strategy_string, RB_prop = 0.5){  
  ...
  if (strategy_string == "RB") {
    ...
    params <- list("prop" = inv.logit(rnorm(1, mean = logit(RB_prop), sd = 0.1)))
    hidden_states = "None"
  } else if (strategy_string == "WSLS") {
    params <- list("noise" = inv.logit(rnorm(1, mean = logit(0.9), sd = 0.1)))
    hidden_states = "None"
  } else if (strategy_string == "SoftmaxTitTat") {
    params <- list("bavioral_temp" = inv.logit(rnorm(1, mean = logit(0.99), sd = 0.2)))
    hidden_states = "None"
...
###Win-Stay-Loose-Switch #!# fiks!
WSLS <- function(params, hidden_states = NULL, player, p_matrix, choice_self, choice_op, return_hidden_states = F){
  #WSLS: Win-stay-loose-switch
  
  #Read in parameters
  stay_prob = params$WSLS_stay_prob
  switch_prob = params$WSLS_switch_prob
  #Read in score from last round
  prev_reward = p_matrix[p_matrix$a1_choice == choice_self]
...
strategies_unprep = c("RB", "SoftmaxTitTat", "0-ToM", "1-ToM", "2-ToM", "3-ToM", "8-ToM") #using and RR and default prep
...
result_df <- compete_all(agent_df, n_rounds = 30, 
...
hidden_states[[100]]$own_hidden_states
...
#Preperation with defaults
strategies = c("RB", "WSLS", "SoftmaxTitTat") #, "0-ToM", "1-ToM", "2-ToM", "3-ToM") #not added yet

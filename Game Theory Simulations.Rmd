...
#Very Nice to do:
  ...
  - Add I.D. to compete_all function
...
pacman::p_load(plyr, tidyverse, raster, reshape2, knitr, brms, boot, rethinking, groupdatp2, patchwork)
```
  ...
  if (level == 0) { #If the (simulated) agent is a 0-ToM
    ...
    for (level_index in 1:level) {
      ...
      mean[level_index] <- decision_function(hidden_states = new_opponent_hidden_states,
        ...
        if (is.infinite(mean[level_index])){
          print(opponent_hidden_states)
          print(new_opponent_hidden_states)
          stop("there is an Inf!! - INFINFINFINFINFINFINFINFINFINF BATMAN!")
          }
...
strategies_unprep <- c("RB", "WSLS", "SoftmaxTitTat", "0-ToM", "1-ToM", "2-ToM", "3-ToM", "7-ToM")

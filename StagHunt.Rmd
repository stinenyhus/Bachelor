...
library(pacman)
p_load(plyr, tidyverse, raster, reshape2, knitr, brms, boot)
```
...
p_matrix <- data_frame(p1_choice = c(1, 0, 1, 0), 
                       p2_choice = c(1, 1, 0, 0), 
                       p1_reward = c(5, 3, 0, 3), 
                       p2_reward = c(5, 0, 3, 3)) 
p_matrix
...
```{r Strategies}
###creating strategies:
  #test=rnorm(n = 100, mean = 0.5, sd = 0.1) #?# an example of a distribition from which it could be sampled

#random choice
random_choice <- function(ID){
  if (is.na(part_df$sample_param[part_df$ID == ID])){ #if the participant does not have a distribution we will make one (this creates individual biases in random judgement) #?# does this make sense?
    #generates individual differences (personal biases)
    part_df$sample_param_mean[part_df$ID == ID] <-  rnorm(1, mean = 0.5, sd = 0.1)
    part_df$sample_param_sd[part_df$ID == ID] <-  rnorm(1, mean = 0.1, sd = 0.01)
  }
  
  p <- part_df[part_df$ID == ID,]
  
  #randomly select rabbit or stag
  choice <- rbinom(1, 1, prob = rnorm(1, mean = p$sample_param_mean, sd = p$sample_param_sd)) 
  return(choice)
}


#Win stay loose switch (biased Nash) - should capture both nash, however this does not differentiate between reward
noise_level_WSLS <- rnorm(1, mean = .10, sd = 0.01)
WSLS <- function(ID){ 
  ...
  } else { #else choose the opposite
    choice <- 1 - last_round$choice[last_round$ID == ID]
  }

  #add noise
  choice <- sample(c(choice, 1-choice), 
         1, 
         prob=c(1-noise_level_WSLS, noise_level_WSLS))
...
part_df <- data_frame(ID=seq(n_p), 
    ...
    sample_param_mean = NA, sample_param_sd = NA)
